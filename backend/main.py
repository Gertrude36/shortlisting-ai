...
#
# This sits at the very outermost layer — below uvicorn, above everything else.
# It stamps CORS headers onto EVERY response including:
#   • Render cold-start 503s (generated by Render's proxy, not our app)
#   • Unhandled 500s that escape all middleware
#   • Starlette/FastAPI startup errors
#
# FastAPICORSMiddleware and _CORSFallbackMiddleware handle the normal path;
# this is purely a safety net for infrastructure-level failures.
# ─────────────────────────────────────────────────────────────────────────────

class RawASGICORSWrapper:
    """
    Outermost ASGI wrapper. Guarantees CORS headers on every HTTP response,
    including responses generated by Render's infrastructure before our app
    is reached (cold-start 503s, health-check failures, etc.).
    """
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # Fast-path: preflight OPTIONS — respond immediately without
        # touching the inner app, guaranteeing headers even on cold start.
        ...
        if not _is_origin_allowed(origin):
            # Not a recognised origin — pass through untouched
            await self.app(scope, receive, send)
            return

        # Intercept send() so we can inject headers before the first
        # "http.response.start" event reaches the client.
        ...
        async def send_with_cors(message: dict) -> None:
            ...
            if message["type"] == "http.response.start" and not headers_injected:
                ...
                # Only set if not already present (FastAPICORSMiddleware may have set them)
                if "access-control-allow-origin" not in mutable:
        ...
        try:
            ...
            # Last-resort: send a plain 500 with CORS headers so the
            # browser doesn't swallow the real error as a CORS failure.
...
# ── Fallback Starlette middleware (belt-and-suspenders inside the app) ────────
...
#
# Desired onion:
#   RawASGICORSWrapper            ← applied manually AFTER app is built (below)
#     _CORSFallbackMiddleware     ← added last  → outermost Starlette layer
#       FastAPICORSMiddleware     ← added first → inner Starlette layer
#         route handler
#
# Add FastAPICORSMiddleware FIRST, then _CORSFallbackMiddleware.
...
# Wrap the entire ASGI app with the raw CORS safety net AFTER all
# middleware has been registered (so it sits outside everything).
app = RawASGICORSWrapper(app)  # type: ignore[assignment]
...
# NOTE: After wrapping with RawASGICORSWrapper, we need a reference to the
# inner FastAPI app to register routes. Use a module-level alias.
_fastapi_app = app.app  # type: ignore[attr-defined]


@_fastapi_app.api_route("/wake", methods=["GET", "HEAD", "OPTIONS"], tags=["health"])
...
@_fastapi_app.api_route("/", methods=["GET", "HEAD"], tags=["health"])
def root():
    return {"status": "ok", "message": "Shortlisting API is running"}


@_fastapi_app.api_route("/health", methods=["GET", "HEAD"], tags=["health"])
def health():
    return {"status": "ok"}


@_fastapi_app.get("/hybridaction/{path:path}", tags=["health"])
...
_fastapi_app.mount("/uploads", StaticFiles(directory=UPLOAD_DIR), name="uploads")
...
@_fastapi_app.post("/auth/register", response_model=TokenResponse, tags=["auth"])
...
@_fastapi_app.post("/auth/login", response_model=TokenResponse, tags=["auth"])
...
@_fastapi_app.get("/auth/me", tags=["auth"])
...
@_fastapi_app.post("/auth/forgot-password", tags=["auth"])
...
@_fastapi_app.post("/auth/reset-password", tags=["auth"])
...
@_fastapi_app.get("/jobs", response_model=List[JobResponse], tags=["jobs"])
def list_jobs(db: Session = Depends(get_db)):
    return db.query(Job).filter(Job.is_active == True).all()

@_fastapi_app.get("/jobs/{job_id}", response_model=JobResponse, tags=["jobs"])
...
@_fastapi_app.post("/jobs", response_model=JobResponse, tags=["jobs"])
def create_job(
    payload: JobCreate,
...
@_fastapi_app.delete("/jobs/{job_id}", tags=["jobs"])
def delete_job(
    job_id: int,
    db: Session = Depends(get_db),
    hr: User    = Depends(require_hr),
...
@_fastapi_app.post("/applications", response_model=ApplicationResponse, tags=["applications"])
...
@_fastapi_app.delete("/applications/{application_id}", tags=["applications"])
...
@_fastapi_app.post("/applications/{application_id}/finalize", tags=["applications"])
...
@_fastapi_app.get("/applications/my", response_model=List[ApplicationResponse], tags=["applications"])
def my_applications(
    db: Session        = Depends(get_db),
    current_user: User = Depends(require_applicant),
):
...
@_fastapi_app.get("/applications/{application_id}", response_model=ApplicationResponse, tags=["applications"])
def get_application(
    application_id: int,
    db: Session        = Depends(get_db),
...
@_fastapi_app.post("/applications/{application_id}/documents", tags=["documents"])
...
@_fastapi_app.get("/applications/{application_id}/documents", tags=["documents"])
def list_documents(
    application_id: int,
    db: Session        = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
@_fastapi_app.delete("/applications/{application_id}/documents/{doc_id}", tags=["documents"])
def delete_document(
    application_id: int,
    doc_id: int,
    db: Session        = Depends(get_db),
...
@_fastapi_app.get("/hr/candidates", tags=["hr"])
...
@_fastapi_app.get("/hr/report/{job_id}", tags=["hr"])
def get_job_report(
    job_id: int,
    db: Session = Depends(get_db),
    hr: User    = Depends(require_hr),
...
@_fastapi_app.post("/hr/shortlist/{application_id}", response_model=ShortlistResult, tags=["hr"])
...
@_fastapi_app.post("/hr/shortlist-all/{job_id}", tags=["hr"])
...
@_fastapi_app.post("/hr/reshortlist/{application_id}", response_model=ShortlistResult, tags=["hr"])
...
@_fastapi_app.post("/hr/reshortlist-all/{job_id}", tags=["hr"])
