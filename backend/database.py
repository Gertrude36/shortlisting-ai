...
def _check_db_connection() -> bool:
    ...
    try:
        ...
        print(f"[database]   Cannot connect to database at startup: {exc}")
        if not _is_sqlite:
            print(
...
def get_db():
    ...
    try:
        ...
        print(f"[get_db]   Failed to create DB session: {exc}")
    ...
    try:
        ...
        print(f"[get_db]   DB OperationalError during request: {exc}")
        try:
            db.rollback()
        except Exception:
            pass
