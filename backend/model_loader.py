...
try:
    ...
    _ARTIFACTS_OK   = False
    print(f"[model_loader]   Failed to load ML artifacts: {_load_exc!r}")
    print("[model_loader]   Server will start in degraded mode (shortlisting disabled).")
...
if os.path.exists(_job_req_path):
    try:
        ...
        job_requirements = {}
        print(f"[model_loader]   job_requirements.pkl found but failed to load: {_jr_exc!r}")
else:
    job_requirements = {}
    print("[model_loader]   job_requirements.pkl not found -- using empty dict (non-critical)")